...
export default function FruVendorConflictPage() {
  const router = useRouter()
  const [columns, setColumns] = useState<Column[]>(defaultColumns)
  const [currentPage, setCurrentPage] = useState(1)
  const [rowsPerPage, setRowsPerPage] = useState(10)
  const [activeTab, setActiveTab] = useState("fru-vendor")
  ...
  }, [selectedFrus, selectedOdms, selectedStatus, createdTimeRange, auditTimeRange])
  ...
  const renderCellContent = (row: ConflictData, key: string) => {
    switch (key) {
      ...
      case "fru":
        return <span className="text-sm font-normal text-foreground font-mono">{row.fru}</span>
